...
# from medseg import eval_loss
# from net import UNET
from net_wbh2 import UNET
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # ====================one train epoch====================
        for i, ((img), (y1, y2)) in enumerate(train_progress):
            img = img.to(device=args.device, dtype=torch.float32)
            y1 = y1.to(device=args.device, dtype=torch.float32)
            y2 = y2.to(device=args.device, dtype=torch.float32)

            segmentation = y1.to(device=args.device, dtype=torch.float32)
            optimizer.zero_grad()
            # print(img.shape)
            pred, fg_pred, bg_pred, uc_pred = model(img)
            # pred = pred.squeeze(1)

            # print(img.shape,pred.shape, fg_pred.shape, bg_pred.shape, uc_pred.shape,y1.shape,y2.shape)
            ...
            beta1 = beta1.to(args.device)
            beta2 = beta2.to(args.device)
            ...
            loss_comp = loss_comp.to(args.device)
            loss = beta1 * loss_fg + beta2 * loss_bg + loss_comp+bceloss(pred, segmentation) + diceloss(pred, segmentation)
        ...
        eval_loss = 0
        ious = []
        for i, ((img), (y1, y2))  in enumerate(eval_progress):
            ...
            with torch.no_grad():
                ...
                loss_comp = complementary_loss(prob_fg, prob_bg, prob_uc)
                loss_comp = loss_comp.to(args.device)
                loss=beta1 * loss_fg + beta2 * loss_bg + loss_comp + bceloss(pred, segmentation) + diceloss(pred,
                                                                                                              segmentation)
                eval_loss+=loss.item()
